...
//
////import Demo.SotaInverseK.JType;
//
//public class AS2_5 {
...
//		double[] leftCenter = MatrixHelp.getTrans(FK.frames.get(Demo.Frames.FrameKeys.L_HAND)).toArray();
//		double[] rightCenter = MatrixHelp.getTrans(FK.frames.get(Demo.Frames.FrameKeys.R_HAND)).toArray();
//		double[] headCenter = MatrixHelp.getTrans(FK.frames.get(Demo.Frames.FrameKeys.HEAD)).toArray();
//
...
//			double[] right = 	new double[]{rightCenter[0] + Math.sin(r_angle)*RADIUS, rightCenter[1], rightCenter[2]+Math.cos(r_angle)*RADIUS};
...
//			RealVector theta = SotaInverseK.solve(Demo.Frames.FrameKeys.L_HAND, Demo.SotaInverseK.JType.O, MatrixUtils.createRealVector(left), currentAngles, MAX_IK_TRIES);
//			theta = SotaInverseK.solve(Demo.Frames.FrameKeys.R_HAND, Demo.SotaInverseK.JType.O, MatrixUtils.createRealVector(right), theta, MAX_IK_TRIES);
//			if (this.TEST_MODE == TestMode.EXTREME)
//				theta = SotaInverseK.solve(Demo.Frames.FrameKeys.HEAD, Demo.SotaInverseK.JType.R, MatrixUtils.createRealVector(head), theta, MAX_IK_TRIES); ;
//
